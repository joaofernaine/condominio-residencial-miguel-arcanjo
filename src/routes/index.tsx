...
import {
  ...
  type AlertasPreferencia,
  fetchAlertasPreferencia,
...
function FuncaoPermissoesDialog({
  ...
  const [tituloFuncao, setTituloFuncao] = useState("");
  const [selecionadas, setSelecionadas] = useState<Set<Permissao>>(new Set());
  const [alertas, setAlertas] = useState<AlertasPreferencia>({ visitante: true, reserva: true });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([fetchPermissoesDoProfile(morador.id), fetchAlertasPreferencia(morador.id)])
      .then(([perms, prefs]) => {
  ...
  const submit = async () => {
    ...
    try {
      await Promise.all([
        ...
        definirAlertasPreferencia(morador.id, alertas),
      ]);
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid gap-2.5 rounded-xl border border-border p-3">
                {PERMISSOES_DISPONIVEIS.map((p) => {
                  const notificaChave = p.id === "aprovar_visitantes" ? "visitante" : p.id === "aprovar_reservas" ? "reserva" : null;
                  return (
                    <div key={p.id} className="flex items-center justify-between gap-2">
                      ...
                      {notificaChave && (
                        <label className="flex shrink-0 cursor-pointer items-center gap-1.5 text-xs text-muted-foreground">
                          <Checkbox
                            checked={alertas[notificaChave]}
                            onCheckedChange={(v) => setAlertas((prev) => ({ ...prev, [notificaChave]: v === true }))}
                          />
